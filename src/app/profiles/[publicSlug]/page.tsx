...
export default async function PublicProfilePage({ params }: { params: Promise<{ publicSlug: string }> }) {
  const { publicSlug } = await params;
  // Anonymous-safe: only ACADEMY_ONLY profiles consult the viewer's academy.
  const session = await getSession();
  const res = await resolvePublicProfile(publicSlug, session?.academyId ?? null);
